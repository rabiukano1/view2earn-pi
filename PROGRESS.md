...
### Auth — web3 wallet login (in progress)
- [x] Server SIWE verification — `getNonce` + `walletAuth` (`convex/wallet.ts`), signature recovery in `@view2earn/core` (`wallet.ts`, `@noble` libs, 5 tests). Identity = wallet address (`externalUid: wallet:0x…`); single-use nonce blocks replay. **Verified end-to-end against the live deployment.**
- [ ] Client wallet connection — Reown AppKit (WalletConnect) in the RN app: needs a free projectId + native rebuild + a login gate replacing the device auto-create. *Not yet wired.*
