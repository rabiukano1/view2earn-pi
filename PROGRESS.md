...
### Auth — Convex Auth (email/password live; Sidra KYC next)
...
- [ ] ~~Google OAuth~~ **removed** — backend provider and `GoogleAuthButton` deleted; sign-in is email/password, email OTP, Telegram, and Pi.
...
- Email OTP sender now uses the verified `no-reply@view2earn.org` domain (Resend verified) — real delivery confirmed; `AUTH_RESEND_FROM` env controls the sender (`ResendOTP.ts`)
