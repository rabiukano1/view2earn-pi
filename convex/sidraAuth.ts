...
// Sidra Chain KYC sign-in (plan §7.1). A future login method: verifies a Sidra
// token against the Sidra API and maps it to a user. Currently unused by the app
// until the Sidra login SDK is integrated on the client.
...
export const sidraAuth = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.scheduler.runAfter(0, internal.sidraAuth.verifySidraToken, {
      userId,
...
export const verifySidraToken = internalAction({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      await ctx.runMutation(internal.sidraAuth.updateUserAfterVerification, {
        userId: args.userId,
