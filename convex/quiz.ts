...
// Nigeria is UTC+1 (West Africa Time, no DST). The daily quiz is "once per day"
// per user, and the day boundary is 00:00 Africa/Lagos — so one quiz resets at
// midnight Nigerian time, consistent with the rest of the reward economy.
const DAY_MS = 24 * 60 * 60 * 1000;
const LAGOS_OFFSET_MS = 1 * 60 * 60 * 1000; // +01:00
...
export const getDailyQuiz = query({
  args: {
    userId: v.id("users"),
    ecosystem: v.union(v.literal("PI"), v.literal("SIDRA")),
    day: v.number(), // kept for API compatibility; the server now derives the
    // Nigerian weekday itself so a user can't change their phone date to get a
    // fresh quiz or pick a different scheduled course.
  },
  handler: async (ctx, { userId, ecosystem, day: _clientDay }) => {
    await requireUser(ctx, userId);

    // Server-side Nigerian day-of-week (0 = Sunday .. 6 = Saturday), computed
    // from Africa/Lagos time — not from the client.
    const now = Date.now();
    const lagosDate = new Date(now + LAGOS_OFFSET_MS);
    ...
    // Auto-seed built-in fallback only when the bank is empty.
    ...
    // Course selection follows quizSettings: MIXED (per-course distribution) or
    // COURSE_OF_THE_DAY (weekday schedule). Selection is driven by the `day` arg
    // so the query stays deterministic and never reads the wall clock.
    const courseByKey = new Map<string, any>(courses.map((c) => [c.key, c]));
    ...
    const pickFromCourse = (courseId: any, n: number) => {
      pick(activeBank.filter((q) => q.courseId === courseId), n);
    };

    const fillFromDistribution = (n: number) => {
      for (const d of distribution) {
        if (chosen.length >= n) break;
        const course = courseByKey.get(d.courseKey);
        if (!course) continue;
        pickFromCourse(course._id, Math.min(d.count, n - chosen.length));
      }
      // Top up with any remaining fresh questions if the distribution under-filled.
      if (chosen.length < n) {
        pick(activeBank, n - chosen.length);
      }
    };

    if (mode === "COURSE_OF_THE_DAY") {
      const todaySchedule = schedule.find((s) => s.day === day);
      const course = todaySchedule ? courseByKey.get(todaySchedule.courseKey) : null;
      if (course) {
        // Pick STRICTLY from the scheduled Course of the Day, then fall back to
        // the rest of the bank only if that course runs short.
        pickFromCourse(course._id, qCount);
        if (chosen.length < qCount) {
          pick(activeBank, qCount - chosen.length);
        }
      } else {
        // No schedule entry (or "MIXED") for this day → fall back to distribution.
        fillFromDistribution(qCount);
      }
    } else {
      // MIXED mode: distribute across the configured courses.
      fillFromDistribution(qCount);
    }

    // Do not pad with repeats: returning duplicate _ids would cause the client
    // (which stores answers by questionId) to silently drop all but one answer.
...
export const getDailyQuizStatus = query({
  ...
  handler: async (ctx, { userId }) => {
    await requireUser(ctx, userId);
    // Nigerian-midnight day boundary (00:00 Africa/Lagos).
    const since = nigerianDayStart(Date.now());
...
export const submitQuiz = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Strictly enforce once per Nigerian calendar day completion (00:00 Lagos).
    const since = nigerianDayStart(Date.now());
