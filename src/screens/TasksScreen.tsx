...
/**
 * Extracts the clean username/handle from a task.
 * Prefers the explicit `name` field (set by the admin panel), then falls back
 * to parsing the target URL. Returns the bare handle WITHOUT the leading "@".
 */
function extractUsername(task: Task): string {
  // Prefer the explicit name field (handle/username) — but skip auto-generated
  // MULTI_TASK titles like "Comment + Repost + Like" which are not handles.
  if (task.name && !task.name.includes(' + ')) {
    return task.name.replace(/^@/, '').trim();
  }
  // Fall back to parsing from the URL
  const url = task.targetUrl || task.steps?.[0]?.targetUrl || '';
  if (!url) return '';
  try {
    const u = new URL(url);
    const parts = u.pathname.replace(/^\/+/, '').replace(/\/+$/, '').split('/');
    const first = parts[0] || '';
    // Strip platform-specific prefixes (channel/, c/, user/, in/, groups/)
    return first
      .replace(/^@/, '')
      .replace(/^(channel|c|user|in|groups)\//, '')
      .trim();
  } catch {
    return '';
  }
...
/** Extracts the username from a MULTI_TASK step. */
function extractStepUsername(step: { name?: string; targetUrl: string }): string {
  if (step.name && !step.name.includes(' + ')) {
    return step.name.replace(/^@/, '').trim();
  }
  try {
    const u = new URL(step.targetUrl);
    const parts = u.pathname.replace(/^\/+/, '').replace(/\/+$/, '').split('/');
    const first = parts[0] || '';
    return first
      .replace(/^@/, '')
...
export default function TasksScreen() {
  ...
  // Copy the task's username/handle to the clipboard for manual follow
  const handleCopyUsername = async (task: Task) => {
    const username = extractUsername(task);
    if (!username) {
      Alert.alert('No Username', 'This task does not have a username to copy.');
      return;
    }
    try {
      ...
      if (Platform.OS === 'android') {
        ...
        Alert.alert('Copied', `@${username} copied to clipboard. Open the app and search for this handle.`);
      }
    } catch {
  ...
  // Copy a MULTI_TASK step's username to the clipboard
  const handleCopyStepUsername = async (step: { name?: string; targetUrl: string }) => {
    const username = extractStepUsername(step);
    if (!username) {
      Alert.alert('No Username', 'This step does not have a username to copy.');
      return;
    }
    try {
      await Clipboard.setString(username);
  ...
  const handleUploadSubmit = async () => {
    ...
    try {
      ...
      // interstitial after successful screenshot upload
    ...
    <View style={[styles.container, dark && styles.containerDark]}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          const isMultiTask = item.type === 'MULTI_TASK' && Array.isArray(item.steps) && item.steps.length > 0;
          ...
          return (
            <View
              ...
              {/* MULTI_TASK: show each step handle with a copy button */}
              {isMultiTask ? (
                <View style={styles.stepsWrap}>
                  {item.steps!.map((step, idx) => {
                    const stepUser = extractStepUsername(step);
                    const actionLabel = (step.label || step.action || 'Step').replace(/_/g, ' ');
                    return (
                      <View key={idx} style={styles.stepRow}>
                        <View style={styles.stepInfo}>
                          <Text style={[styles.stepAction, dark && styles.textLight]} numberOfLines={1}>
                            {actionLabel}
                          </Text>
                          {stepUser ? (
                            <Text style={styles.stepHandle} numberOfLines={1}>
                              @{stepUser}
                            </Text>
                          ) : null}
                        </View>
                        {stepUser ? (
                          <TouchableOpacity
                            style={styles.stepCopyBtn}
                            activeOpacity={0.8}
                            onPress={() => handleCopyStepUsername(step)}>
                            <Icon name="copy" iconStyle="solid" size={12} color={colors.primary} />
                            <Text style={styles.stepCopyText}>Copy</Text>
                          </TouchableOpacity>
                        ) : null}
                      </View>
                    );
              ...
              <View style={styles.taskActionRow}>
                {isDone ? (
                  ...
                  <View style={styles.pendingActionGroup}>
                    ...
                    {username ? (
                      <TouchableOpacity
                        ...
                        <Icon name="copy" iconStyle="solid" size={12} color={colors.primary} />
                        <Text style={styles.copyHandleText}>Copy @{username}</Text>
                      </TouchableOpacity>
                    ) : null}
                    <TouchableOpacity
                  ...
                  <View style={styles.availableActionGroup}>
                    {username ? (
                      <TouchableOpacity
                        style={styles.copyHandleBtn}
                        activeOpacity={0.8}
...
const styles = StyleSheet.create({
  ...
  // Copy handle button (manual follow)
  copyHandleBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
