...
export default function SpinScreen() {
  ...
  // pendingSpin holds the server prize until the wheel stops and user claims
  const pendingSpinRef = useRef<{ spinId: string; pts: number; prizeIndex: number } | null>(null);
  ...
  const executeSpin = async (force = false) => {
    ...
    setHasPending(false);
    try {
      const { spinId, pts, prizeIndex } = await doSpin({ userId });
      pendingSpinRef.current = { spinId: spinId as string, pts, prizeIndex };
      setHasPending(true);

      const targetIndex = prizeIndex;
      // exact mathematical target angle calculates bisector of target 36° sector aligned at top 12 o'clock pointer + 6 full 360° revolutions.
      ...
      Animated.timing(wheelAnimatedValue, {
        ...
        // Wheel stopped on exact server prizeIndex — show preview, credit happens on claim
        setResult(pts);
        setSpinning(false);
        startCelebration();
      });
  ...
  const doClaim = async (doubled: boolean) => {
    const pending = pendingSpinRef.current;
