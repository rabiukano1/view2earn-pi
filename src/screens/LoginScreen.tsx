...
import {
  consumeAttributedReferrer,
  subscribeToReferralAttribution,
...
export default function LoginScreen({ onShowSplash }: LoginScreenProps = {}) {
  ...
  // Auto-attribution: when the app was installed/opened via a referral link
  // (Play Store `referrer=` or `view2earn://...?referrer=...`), capture the code
  // and apply it automatically after signup — no manual entry needed.
  useEffect(() => {
    let mounted = true;
    consumeAttributedReferrer()
      .then((ref) => {
        if (ref && mounted && !referralApplied) {
          const code = ref.trim().toUpperCase();
          setReferralCode(code);
        }
      })
      .catch(() => {});
    const unsubscribe = subscribeToReferralAttribution((ref) => {
      if (ref && !referralApplied) {
...
const styles = StyleSheet.create({
  ...
  eyeBtn: { padding: 8, marginHorizontal: -4, justifyContent: 'center', alignItems: 'center' },
